/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { VerificationUpdateManyWithoutAdministratorsInput } from "./VerificationUpdateManyWithoutAdministratorsInput";
import { Type } from "class-transformer";
import { TaskUpdateManyWithoutAdministratorsInput } from "./TaskUpdateManyWithoutAdministratorsInput";

@InputType()
class AdministratorUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  email?: string | null;

  @ApiProperty({
    required: false,
    type: () => VerificationUpdateManyWithoutAdministratorsInput,
  })
  @ValidateNested()
  @Type(() => VerificationUpdateManyWithoutAdministratorsInput)
  @IsOptional()
  @Field(() => VerificationUpdateManyWithoutAdministratorsInput, {
    nullable: true,
  })
  verifications?: VerificationUpdateManyWithoutAdministratorsInput;

  @ApiProperty({
    required: false,
    type: () => TaskUpdateManyWithoutAdministratorsInput,
  })
  @ValidateNested()
  @Type(() => TaskUpdateManyWithoutAdministratorsInput)
  @IsOptional()
  @Field(() => TaskUpdateManyWithoutAdministratorsInput, {
    nullable: true,
  })
  tasks?: TaskUpdateManyWithoutAdministratorsInput;
}

export { AdministratorUpdateInput as AdministratorUpdateInput };
